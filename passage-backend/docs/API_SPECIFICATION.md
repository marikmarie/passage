...
**Base URL:** `http://localhost:8000/api/v1` (or deployed domain)
...
The plain-PHP implementation uses authenticated REST endpoints for watch location ingestion and client polling:

- Watch state: `GET /api/v1/watch/state/{deviceId}`
- Watch location: `POST /api/v1/devices/location`
- Parent/rider latest location: `GET /api/v1/tracking/latest/{deviceId}`
- Parent/rider playback: `GET /api/v1/tracking/playback/{deviceId}`

The PHP application uses REST state and tracking endpoints. If websocket push is needed in production, deploy it as a separate, explicitly designed long-running PHP service rather than adding a framework to this application.
